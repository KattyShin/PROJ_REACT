...
import UpdateAccModal from './Modal/UpdateAccModal';
...
function Accounts(){
    ...
    //Hide some characters in password
    ...
    return(
        <div className="main"> 
            ...
            <div className="accs-parent-div w-full h-full">
                    ...
                    <div className="acc-div flex w-full">
                        ...
                        {adminData ? (
                            <div className="adm-acc-div pt-[4vh] pr-[30vh] w-full">
                                <label>Username: <label className="admin-lbl pl-[4px]">{adminData.username}</label> </label><br />
                                <label>Password: <label className="admPass-lbl pl-[10px]">{maskPassword(adminData.password)}</label> </label>
                            </div>
                        ) : (
                            <p>Loading Admin Data</p>
                        )}
                        <div className="btn-div pt-[4vh] w-full">
                            <button onClick={() => openUpdAccModal(12134)} className="adm-upd-acc-btn text-white bg-accent hover:bg-slate-400 w-[50%] h-[50%] rounded-md">Update</button>
                        </div>
                    </div> 
                </div> <br />
                <div className="acc-txt-div">
                ...
                <div className="acc-parent flex flex-col w-[97%] bg-sidebar-bg h-[30%] justify-center">
                    <div className="acc-div flex w-full">
                        ...
                        {staffData ? (
                            <div className="adm-acc-div pt-[4vh] pr-[30vh] w-full">
                                <label>Username: <label className="staff-lbl pl-[4px]">{staffData.username}</label> </label><br />
                                <label>Password: <label className="stPass-lbl pl-[10px]">{maskPassword(staffData.password)}</label> </label>
                            </div>
                        ) : (
                            <p>Loading Staff Data</p>
                        )}
                        <div className="btn-div pt-[4vh] w-full">
                            <button onClick={() => openUpdAccModal(14563)} className="st-upd-acc-btn text-white bg-accent hover:bg-slate-400 w-[50%] h-[50%] rounded-md">Update</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {isUpdAccountModalVisible && selectedUser && (
                <UpdateAccModal 
                    user={selectedUser}
                    onClose={closeUpdAccModal}
                    onUpdate={handleUpdate}
                />
            )}

            {isSuccessfullyModal && (
                <Successfully onConfirm={hideSuccessfullyModal} />
            )}
        </div>
    )
}
